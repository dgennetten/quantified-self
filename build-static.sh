...
echo "Building static version for Dreamhost..."

# Clean previous builds
rm -rf static-build

# First, build the React app properly
cd client
npm install
npm run build
cd ..
...
# Copy the built React files (this should include index.html and all JS/CSS)
cp -r client/build/* static-build/

# Create the mock API script
cat > static-build/api-mock.js << 'EOF'
...
# Add the mock API script to the index.html
# Find the closing </body> tag and insert our script before it
sed -i 's|</body>|<script src="api-mock.js"></script></body>|' static-build/index.html

echo "Static build created in 'static-build' directory!"
echo "Files in static-build:"
ls -la static-build/

echo ""
echo "Upload the contents of 'static-build' to your Dreamhost domain."
echo ""
echo "Demo credentials:"
echo "Email: douglas@gennetten.com"
echo "Password: any password (6+ characters)"
